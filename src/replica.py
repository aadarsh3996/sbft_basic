...
def start_transaction(data=None):
    ...
    if primary_info["port"] != p:
        ...
        try:
            d = requests.post(create_url(
                primary_info["ip"], primary_info["port"], "pre_prepare"), json=data, timeout=1)
        except:
    ...
    for node in network_info.values():
        try:
            if node["node_type"] != "primary":
                d = requests.post(create_url(
                    node["server_ip"], node["port"], "prepare"), json=data, timeout=1)
            else:
                prepare(data)
        except:
            pass
...
def view_change_trigger(data=None):
    ...
    for node in nodes_info.values():
        try:
            d = requests.post(create_url(
                node["server_ip"], node["port"], "elect_new_leader"), json=msg, timeout=1)
        except:
            pass
...
def elect_new_leader(data=None):
    ...
    for node in nodes_info.values():
        try:
            d = requests.post(create_url(
                node["server_ip"], node["port"], "view_change"), json=msg, timeout=1)
        except:
            pass
...
def view_change(data=None):
    ...
    if len(votes) == 2*f + 2*c + 1:
        ...
        try:
            d = requests.post(create_url(
                leader_node["server_ip"], leader_node["port"], "create_new_view"), json=msg, timeout=1)
        except:
            pass
...
def prepare(data=None):
    ...
    if flag:
        ...
        try:
            r = requests.post(create_url(host, port, route), json=data, timeout=1)
        except:
            pass
...
def c_collector(data=None):
    ...
    if hash!=c_hash:
        ...
        return "<p>Hello, World!</p>"
    data["hash"] = c_hash
    print("Adding messages to c_collector_queue {}".format(
        data["transaction_id"]))
    ...
    if len(c_collector_queue[data["transaction_id"]]) >= 2*f+c+1:
        ...
        for node in network_info.values():
            if node['port'] != p:
                try:
                    k = requests.post(create_url(
                        node["server_ip"], node["port"], "commit_proof"), json=v, timeout=1)
                except:
                    pass
            else:
                commit_proof(v)
...
def commit_proof(data=None):
    ...
    for n in network_info.values():
        if n["node_type"] == "e_collector" and n["port"] != p:
            try:
                te = requests.post(create_url(
                    n["server_ip"], n["port"], "e_collector"), json=data, timeout=1)
            except:
                pass
            break
        if n["node_type"] == "e_collector" and n["port"] == p:
    ...
    if len(e_collector_queue[data["transaction_id"]]) >= f+c+1:
        ...
        for node in network_info.values():
            if node['port'] != p:
                try:
                    k = requests.post(create_url(
                        node["server_ip"], node["port"], "execute_proof"), json=v, timeout=1)
                except:
                    pass
            else:
                execute_proof(v)
